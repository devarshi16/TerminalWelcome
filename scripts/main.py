...
from data.one_liners import one_liners
...
def main():
    ...
    parser.add_argument('-p','--pokemon',help='pokemon name for ASCII art. 0 for disabling ASCII art',choices=pokemons.keys())
    ...
    default_config_path = os.path.join(curr_path,'config','poketermconfig.ini')
    ...
    if args.show:
        ...
        if local_config["DEFAULTS"]["message"] == None or local_config["DEFAULTS"]["message"] == "none" or local_config["DEFAULTS"]["message"]=="None":
            ...
            print(local_config["message"])

        # show ascii art
        if local_config["DEFAULTS"]["ascii"]:
        ...
        if local_config["DEFAULTS"]["one-liner"]:
